...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    headers: function headers() {
      return [{
        key: 'name',
        label: this.$t('dealers.name'),
        sortable: true
      }, {
        key: 'owner',
        label: this.$t('dealers.owner'),
        sortable: true
      }];
    },
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                _this.loading = true;
                includes = ['users'];
                query = new _services_api_DealersService__WEBPACK_IMPORTED_MODULE_8__["default"]().paginate(page, perPage).filter(_this.search).sort(orderBy, sortedBy).include(includes).all();
                _context.next = 6;
                return query;
